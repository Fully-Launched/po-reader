...
- **`inventoryLocationId`** — resolved the same way as `businessUnitId`: a dropdown in the review table (`GET /api/inventory-locations` → `ServiceTitanClient.listInventoryLocations()`), the user picks a name, never a raw ID. **The endpoint changed once already**: the original guess (`GET .../inventory-locations`) 404'd live ("Unable to match incoming request to an operation" — that resource doesn't exist). It's now `GET /inventory/v2/tenant/{tenant}/warehouses`, based on real (if secondary-source, still not ServiceTitan's own live docs) research — see the full research trail in the method's doc comment in `client.ts`. Two independent third-party sources (a ServiceTitan API scope profile, and a reconstructed OpenAPI spec) agree that what the product UI calls "Inventory Locations" is the "Warehouses" resource in the API. Still not live-verified — test against the sandbox before relying on it.
...
- [ ] **`inventoryLocationId` endpoint** — `listInventoryLocations()` now hits `GET /inventory/v2/tenant/{tenant}/warehouses` (corrected from an earlier guess that 404'd; see the research trail in `client.ts`'s doc comment on that method). Backed by two independent third-party sources agreeing "Warehouses" is the right resource, not ServiceTitan's own live docs — still needs testing against the sandbox tenant. Also unconfirmed: whether Comfort x Design has one Inventory Location or several (mirrors the same open question for Business Units).
