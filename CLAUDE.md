...
- **`impactsTechnicianPayroll`** — hardcoded to `false` in `buildPoPayload()`. **Confirmed on a client call, no change**: these invoices have no job or technician attached (see "Job attachment is optional" below), so there's no payroll to impact.
...
- **`shipTo`** — shape CONFIRMED via a live 400: it's a nested wrapper, `{ address: {...}, description: "..." }`, not a flat address object as first guessed. A follow-up live 400 further confirmed the nested `address` object's `unit` field must not be `null` ("shipTo.Address.Unit is required") — fixed by sending `unit: ""` instead. Both fixes are in `CLIENT_SHIP_TO_ADDRESS`/`buildPoPayload()` in `payload-builder.ts`, and the full shape is what PO #19969 succeeded against. **The address CONTENT is CONFIRMED REAL, not a placeholder** — confirmed on a client call: `"Comfort X Design, Inc." / 510 S Spring Road / Elmhurst, IL 60126`, no unit/suite. Renamed from `SANDBOX_PLACEHOLDER_ADDRESS` to `CLIENT_SHIP_TO_ADDRESS` to reflect this — it's the real address to use in every environment, not sandbox-only. `shipTo.description` (currently `"Sandbox test PO"`, `PLACEHOLDER_SHIP_DESCRIPTION`) is a separate, still-unconfirmed field — the client call confirmed the address, not the description.
...
- [x] **Shipping/receiving address for `shipTo`** — CONFIRMED on a client call: `510 South Spring Road, Elmhurst, IL 60126`, no unit/suite. This is Comfort x Design's real address, not a sandbox placeholder — `payload-builder.ts`'s constant was renamed from `SANDBOX_PLACEHOLDER_ADDRESS` to `CLIENT_SHIP_TO_ADDRESS` to reflect this, and it's used in every environment, not sandbox-only. (`shipTo.description` remains a separate, still-unconfirmed placeholder — not covered by this confirmation.)
...
- [x] Sandbox PO creation working end-to-end — CONFIRMED: a real end-to-end test succeeded, PO #19969 created in the ServiceTitan sandbox tenant from the real 56-line-item Arco Supply invoice, all 56 line items included, correctly auto-received (status `Received`, not just `Sent`), Business Unit and Inventory Location both resolved via the live dropdowns, total `$4,411.64` matching the source invoice exactly.
