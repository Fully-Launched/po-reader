...
- **`requiredOn`** — hardcoded to today's date (`new Date().toISOString().slice(0, 10)`) in `buildPoPayload()`. Not yet user-facing — this is a placeholder just to give ServiceTitan a valid value, not a real requested-by date. Revisit once/if this needs to be meaningful (e.g. a vendor's quoted lead time) — still open, see Open Questions.
- **`shipTo`** — shape CONFIRMED via a live 400: it's a nested wrapper, `{ address: {...}, description: "..." }`, not a flat address object as first guessed. A follow-up live 400 further confirmed the nested `address` object's `unit` field must not be `null` ("shipTo.Address.Unit is required") — fixed by sending `unit: ""` instead. Both fixes are in `SANDBOX_PLACEHOLDER_ADDRESS`/`buildPoPayload()` in `payload-builder.ts`, and the full shape is what PO #19969 succeeded against. **The address CONTENT itself (`"Comfort X Design, Inc." / 510 S Spring Road / Elmhurst, IL 60126`) is still a sandbox placeholder pulled from ad-hoc test data, not Comfort x Design's real shipping/receiving address** — must be replaced before production use, see Open Questions.
- **`shipping`** — shape CONFIRMED via a live 400 ("Could not convert string to decimal: Ground"): it's a plain **numeric** field (a freight/shipping cost as a decimal), not an address object and not a carrier/method string as first guessed. Currently hardcoded to `0` in `buildPoPayload()`, matching the sample invoice's own `FREIGHT: 0.00` line. **Real freight-cost handling (e.g. reading a nonzero freight amount off the invoice, if any vendor charges it) is still open** — see Open Questions.

On the generic **`"request"`** error that appeared alongside these: CONFIRMED resolved by sending an empty object, `request: {}`, as a top-level field — once shipTo/shipping were fixed and `request: {}` was added, PO #19969 succeeded with no further error naming `request`. This is consistent with (though not independently proven beyond) the original hypothesis that it was ASP.NET `[ApiController]` duplicate-validation-key noise tied to the other missing fields rather than a real required field with meaningful content — empirically, an empty object satisfies whatever check was firing. Left as `request: {}` in `buildPoPayload()`; no further action needed unless a future error names something specific expected inside it.
...
- [ ] **Shipping/receiving address for `shipTo` — HIGH PRIORITY, shape confirmed, but the CONTENT is still a sandbox placeholder, not Comfort x Design's real address.** `payload-builder.ts`'s `SANDBOX_PLACEHOLDER_ADDRESS` (`"Comfort X Design, Inc." / 510 S Spring Road / Elmhurst, IL 60126`) was pulled from ad-hoc test invoice data purely to unblock sandbox testing. Need from the client: the real address materials should ship to / be received at — is it one fixed address, or does it vary by Business Unit / Inventory Location / job? **Do not use the current placeholder for anything beyond sandbox testing.**
- [ ] **Real freight/shipping cost handling** — `shipping` (now confirmed numeric) is hardcoded to `0`, matching the sample invoice's own `FREIGHT: 0.00` line. Not yet confirmed whether any vendor invoice actually carries a nonzero freight charge that should flow through here instead of always being `0`.
...
- [ ] Real shipping/receiving address obtained from client and `SANDBOX_PLACEHOLDER_ADDRESS` replaced (shape is now confirmed; the address content is still a guess pulled from test data — see Open Questions)
